...
int generate_makefile(orbis_imports_t **imports, int imports_count);
...
int main(int argc, const char **argv)
{
	...
	orbis_imports_t **imports = malloc(sizeof(orbis_imports_t*) * imports_count);

	int i;
	for (i = 0; i < imports_count; i++)
	{
		orbis_imports_t *imp = orbis_imports_load(argv[i + 1], 1);
...
int generate_assembly(orbis_imports_t **imports, int imports_count)
{
	...
	for (h = 0; h < imports_count; h++) {
		orbis_imports_t *imp = imports[h];

		for (i = 0; i < imp->n_libs; i++) {
			orbis_imports_lib_t *library = imp->libs[i];
			for (j = 0; j < library->n_modules; j++) {
				orbis_imports_module_t *module = library->modules[j];

				for (k = 0; k < module->n_functions; k++) {
					orbis_imports_stub_t *function = module->functions[k];
					...
					fprintf(fp, ".section .orbislink.fstubs.%s,\"ax\",%%progbits\n\n", module->name);
					fprintf(fp,
						"\t.align 4\n"
						"\t.global %s\n"
						"\t.type %s, %%function\n"
				...
				for (k = 0; k < module->n_variables; k++) {
					orbis_imports_stub_t *variable = module->variables[k];
					...
					if ((fp = fopen(filename, "w")) == NULL)
						return 0;
					fprintf(fp, ".arch armv7a\n\n");
					fprintf(fp, ".section .orbislink.vstubs.%s,\"aw\",%%progbits\n\n",module->name);
					fprintf(fp,
						"\t.align 4\n"
						"\t.global %s\n"
						"\t.type %s, %%object\n"
...
void write_cmake_sources(FILE *fp, const char *modname, const char *postfix, orbis_imports_module_t *library)
{
	int k;

	for (k = 0; k < library->n_functions; k++) {
		orbis_imports_stub_t *function = library->functions[k];
		fprintf(fp, "\t\"%s_%s_%s%s.S\"\n", modname, library->name, function->name, postfix);
	}
	for (k = 0; k < library->n_variables; k++) {
		orbis_imports_stub_t *variable = library->variables[k];
		fprintf(fp, "\t\"%s_%s_%s%s.S\"\n", modname, library->name, variable->name, postfix);
	}
}

int generate_cmake_user(FILE *fp, const char *postfix, orbis_imports_lib_t *module)
{
	int i;
	int found_libs = 0;
	...
	{
		orbis_imports_module_t *library = module->modules[i];
...
int generate_cmake_kernel(FILE *fp, const char *modname, const char *postfix, orbis_imports_module_t *library)
{
...
int generate_cmake(orbis_imports_t **imports, int imports_count)
{
	...
	for (h = 0; h < imports_count; ++h)
	{
		...
		for (i = 0; i < imp->n_libs; i++)
		{
			orbis_imports_lib_t *module = imp->libs[i];
			...
			{
				orbis_imports_module_t *library = imp->libs[i]->modules[j];
...
int generate_makefile(orbis_imports_t **imports, int imports_count)
{
	...
	for (h = 0; h < imports_count; h++) {
		...
		for (i = 0; i < imp->n_libs; i++) {
			...
			for (j = 0; j < imp->libs[i]->n_modules; j++) {
				orbis_imports_module_t *module = imp->libs[i]->modules[j];
...
{
	fprintf(stderr,
		"orbis-libs-gen by xerpi\n"
		"usage: orbis-libs-gen [-c] nids.yml [extra.yml ...] output-dir\n"
